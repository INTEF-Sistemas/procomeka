const e=[`CREATE TABLE IF NOT EXISTS "user" (
		...
		created_at TIMESTAMP NOT NULL DEFAULT NOW(),
		updated_at TIMESTAMP NOT NULL DEFAULT NOW()
	)`,"CREATE INDEX IF NOT EXISTS idx_resources_slug ON resources(slug)","CREATE INDEX IF NOT EXISTS idx_resources_status ON resources(editorial_status)","CREATE INDEX IF NOT EXISTS idx_resources_type ON resources(resource_type)","CREATE INDEX IF NOT EXISTS idx_taxonomies_type ON taxonomies(type)",`DO $$ BEGIN
		ALTER TABLE "resources" ADD COLUMN created_by TEXT REFERENCES "user"(id);
	EXCEPTION WHEN duplicate_column THEN NULL;
	END $$`];async function N(T){for(const E of e)await T.exec(E)}export{e as SCHEMA_STATEMENTS,N as createTables};
